...
contract EfficientInnerProductVerifier {
    ...
    constructor(address _publicParameters) public {
    ...
    function verify(
        uint256 c_x,
        uint256 c_y,
        uint256[2*n] memory ls,
        uint256[2*n] memory rs,
        uint256 A,
        uint256 B
    ) public
    view
    returns (bool) {
        return verifyWithCustomParams(alt_bn128.G1Point(c_x, c_y), ls, rs, A, B, publicParameters.gs(),
        publicParameters.hs(), publicParameters.H());
    }
    ...
    function multiExp(uint256[m] memory ss, alt_bn128.G1Point[m] memory gs) internal view returns (alt_bn128.G1Point memory g) {
        g = gs[0].mul(ss[0]);
    ...
    function multiExpInversed(uint256[m] memory ss, alt_bn128.G1Point[m] memory hs) internal view returns (alt_bn128.G1Point memory h) {
        h = hs[0].mul(ss[m-1]);
        for (uint256 i = 1; i < m; i++) {
